package com.bb.vib.binidngadapters
...
import com.bb.vib.extentions.getParentActivity
import com.bb.vib.extentions.getString
...
fun setUrl(view: ImageView, url1: String?, placeholder: Drawable?, circle: Boolean) {
    ...
    if (!url.contains("http")) {
        url = "${com.bb.vib.api.Constants.BASE_URL}$url"
    }
    if (url.contains(".pdf") || url.contains(".PDF")) {
        if (circle)
            Glide.with(view).load(R.mipmap.ic_vib_launcher).circleCrop().into(view)
        else
            Glide.with(view).load(R.mipmap.ic_vib_launcher).into(view)
...
fun showSnackBar(view: View, snackbar: MutableLiveData<String>?) {
    view.getParentActivity()?.let {
        snackbar?.observe(it) { it1 ->
            ...
            snackbar.view.setBackgroundColor(view.resources.getColor(R.color.app_white))
            val textView = snackbar.view.findViewById<TextView>(R.id.snackbar_text)
            textView.setTextColor(view.resources.getColor(R.color.app_black))
            snackbar.show()
        }
    }
}
